export const PROMPT_LIBRARY = {
  beginner: [
    "Explain this topic like I am moving from admin to developer, then give me one safe practice task.",
    "Show me the minimum correct LWC structure before adding advanced patterns.",
    "Give me one Apex example and one likely interview follow-up question."
  ],
  "admin-to-dev": [
    "Map this topic to what an admin already knows from flows, page layouts, and validation rules.",
    "Show me how this concept appears in a real org requirement, not just an interview answer.",
    "Give me one conversion exercise from declarative logic to code."
  ],
  interview: [
    "Ask me a tough interview version of this topic and then grade my answer.",
    "Give me common mistakes an interviewer expects me to avoid.",
    "Turn this concept into a 90-second spoken answer."
  ],
  "project-builder": [
    "Turn this concept into a mini feature with component, server logic, and testing notes.",
    "Give me a clean file structure before I start coding.",
    "Explain the performance and security choices for this implementation."
  ]
};

export const MEDIA_LIBRARY = {
  lwc: [
    {
      type: "Video",
      title: "LWC learning preview",
      description: "Short playable preview card for the in-app learning shelf.",
      href: "https://developer.salesforce.com/developer-centers/lightning-web-components",
      previewVideo: "https://interactive-examples.mdn.mozilla.net/media/cc0-videos/flower.mp4",
      image:
        "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?auto=format&fit=crop&w=900&q=80"
    },
    {
      type: "Podcast",
      title: "The Salesforce Developers Podcast",
      description: "Useful for ecosystem context, platform patterns, and career-oriented conversations.",
      href: "https://developer.salesforce.com/podcast",
      image:
        "https://images.unsplash.com/photo-1478737270239-2f02b77fc618?auto=format&fit=crop&w=900&q=80"
    },
    {
      type: "Article",
      title: "LWC developer guide",
      description: "Official reading source for Lightning Web Components concepts and examples.",
      href: "https://developer.salesforce.com/docs/platform/lwc/guide",
      image:
        "https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&w=900&q=80"
    },
    {
      type: "Photo",
      title: "Component workspace reference",
      description: "Visual reference for study cards, code workspaces, and component-thinking sessions.",
      href: "https://unsplash.com/s/photos/web-development",
      image:
        "https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&w=900&q=80"
    },
    {
      type: "Video",
      title: "Developer concepts preview",
      description: "Playable preview card for design, structure, and component-oriented study sessions.",
      href: "https://developer.salesforce.com/video",
      previewVideo: "https://www.w3schools.com/html/mov_bbb.mp4",
      image:
        "https://images.unsplash.com/photo-1516321497487-e288fb19713f?auto=format&fit=crop&w=900&q=80"
    }
  ],
  apex: [
    {
      type: "Video",
      title: "Apex learning preview",
      description: "Short playable preview card for the in-app learning shelf.",
      href: "https://developer.salesforce.com/developer-centers/apex",
      previewVideo: "https://interactive-examples.mdn.mozilla.net/media/cc0-videos/flower.mp4",
      image:
        "https://images.unsplash.com/photo-1515879218367-8466d910aaa4?auto=format&fit=crop&w=900&q=80"
    },
    {
      type: "Podcast",
      title: "The Salesforce Developers Podcast",
      description: "Good for hearing how experienced developers think about maintainable server-side design.",
      href: "https://developer.salesforce.com/podcast",
      image:
        "https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&w=900&q=80"
    },
    {
      type: "Article",
      title: "Apex developer guide",
      description: "Official reading source for Apex language concepts and server-side development.",
      href: "https://developer.salesforce.com/docs/atlas.en-us.apexcode.meta/apexcode/",
      image:
        "https://images.unsplash.com/photo-1515879218367-8466d910aaa4?auto=format&fit=crop&w=900&q=80"
    },
    {
      type: "Photo",
      title: "Backend workspace reference",
      description: "Visual reference for server-side study, debugging, and architecture thinking.",
      href: "https://unsplash.com/s/photos/backend-development",
      image:
        "https://images.unsplash.com/photo-1515879218367-8466d910aaa4?auto=format&fit=crop&w=900&q=80"
    },
    {
      type: "Video",
      title: "Server-side concepts preview",
      description: "Playable preview card for architecture, patterns, and backend-oriented study sessions.",
      href: "https://developer.salesforce.com/video",
      previewVideo: "https://www.w3schools.com/html/mov_bbb.mp4",
      image:
        "https://images.unsplash.com/photo-1522202176988-66273c2fd55f?auto=format&fit=crop&w=900&q=80"
    }
  ]
};
